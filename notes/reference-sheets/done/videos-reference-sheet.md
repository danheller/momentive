# Videos Rebuild — Reference Sheet (4 posts, updated 2026-08-19)

Decoded content and architecture notes for the legacy `videos` CPT.
...
**Plus, as of 2026-08-19:** the sole `video-testimonials` post ("Modernizing veterinary
regulation with Nimble AMS," legacy permalink `/testimonials/ams-abvma/`) folds in here too
— see `video-testimonials-reference-sheet.md` for the full reversal writeup. It was
originally planned to fold into `testimonial`, but that CPT is registered non-public with no
rewrite (`inc/testimonials.php`), and this post has a real singular page — hero,
`resource_details`, checklist, CTA, "watch now" button — the exact shape already documented
below, not a quote fragment. It's now in scope for this CPT's migration script as a 4th post,
same field map as the other 3, no schema change needed. The only content difference is tone
(named-customer success story vs. product-demo copy) — not structural.

**What this is:** a minimal "watch this video" landing page — a Wistia embed plus a short description and checklist, no HubSpot form, no presenters, no date. 3 of the 4 posts are MIP Accounting product videos; the 4th (video-testimonial fold-in) is a Nimble AMS customer story, Association Management category. `notes/todo.txt` already flagged this as "no form, just a watch now button" and floated folding into webinars or a minimal standalone CPT — this reference sheet confirms that read and adds detail to decide between the two.
