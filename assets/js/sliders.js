...
( function () {
	...
	function getContentLeftOffset() {
		const contentEl = document.querySelector( '.fine-print' );
		if ( ! contentEl ) return 0;
		return contentEl.getBoundingClientRect().left;
	}

	function getLastAllowedIndex( sp ) {
		const contentEl = document.querySelector( '.is-layout-constrained' );
		if ( ! contentEl ) return sp.length;
		...
		const firstSlide = sp.Components.Elements.slides[0];
		if ( ! firstSlide ) return sp.length;

		const slideWidth = firstSlide.offsetWidth +
			parseFloat( getComputedStyle( firstSlide ).marginRight );

		const visibleCount = Math.floor( availableWidth / slideWidth );
		return sp.length - visibleCount;
	}

	// Wrap each child matching `childSelector` in <li class="splide__slide">,
	// inside a generated track/list, and hand off to setupslider.
	// Used for .solutions-slider and .testimonials-slider, which previously
	// had byte-identical setup blocks.
	function buildSliderFromChildren( el, childSelector, num ) {
		const slides = el.querySelectorAll( childSelector );
		if ( ! slides.length ) return;

		el.classList.add( 'splide', 'slider' );
		el.id = 'splide-number-' + num;

		const track = document.createElement( 'div' );
		track.classList.add( 'splide__track', 'alignfull' );
		el.append( track );
	...
	// ── Auto-scrolling slider (e.g. .autoslider) ──────────────────────────────

	function setupautosliding( el, splidenum ) {
		const slides = el.querySelectorAll( 'figure' );
		if ( ! slides.length ) return;

		// Build a new container next to the original element. The original
		// gets removed at the end since its children are moved into the
		// generated <li class="splide__slide"> wrappers.
		const container = document.createElement( 'div' );
		container.className = el.className + ' splide slider';
		container.id        = 'splide-number-' + splidenum;
		...
		if ( el.classList.contains( 'right' ) ) {
			options.autoScroll.speed = -0.75;
		}

		el.remove();

		splide[ splidenum ] = new Splide( '#splide-number-' + splidenum, options )
			.mount( window.splide.Extensions );
	}

	// ── Standard slider used by solutions, testimonials, news ─────────────────
...
//			options.drag       = 'free';
		...
		if ( which.classList.contains( 'autoplay-on-enter' ) ) {
			...
			// Bounce direction at boundaries for all autoplay-on-enter sliders
	...
	document.querySelectorAll( '.autoslider' ).forEach( el => {
		setupautosliding( el, splidenumber++ );
	} );

	document.querySelectorAll( '.solutions-slider' ).forEach( el => {
	...
	document.querySelectorAll( '.news-slider' ).forEach( el => {
		...
		const track = el.querySelector( '.wp-block-query' );
