...
( function () {
	...
	let splidenumber = 0;
	let splide = new Array();

	// autosliders
	let autosliders = document.querySelectorAll(".autoslider");
	autosliders.forEach( function( auto ) {
		setupautosliding( auto, splidenumber);
		splidenumber++;
	});

	// solution slider
	var solutions = document.querySelectorAll(".solutions-slider");
	solutions.forEach( function( sol ) {
		sol.classList.add("splide","slider");
		sol.id = "splide-number-" + splidenumber;
	
		// track
		const eltrack = document.createElement("div");
		eltrack.classList.add("splide__track","alignfull");
		sol.append( eltrack );
	
		// list
		const ellist = document.createElement("ul");
		ellist.classList.add("splide__list");
		eltrack.append( ellist );

		slides = sol.querySelectorAll(".solution");
		
		if( slides[0] ) {
			slides.forEach( function( sl ) {
				var sli = document.createElement("li");
				sli.classList.add("splide__slide");
				sli.append( sl );
				ellist.append( sli );
			});
		}

		setupslider( sol, splidenumber);
		splidenumber++;
	});

	// testimonial slider
	var testimonials = document.querySelectorAll(".testimonials-slider");
	testimonials.forEach( function( tes ) {
		tes.classList.add("splide","slider");
		tes.id = "splide-number-" + splidenumber;
		...
		slides = tes.querySelectorAll(".testimonial");
		...
		setupslider( tes, splidenumber);
		splidenumber++;
	});

	// news slider
	var newsslider = document.querySelectorAll(".news-slider");
	newsslider.forEach( function( nws ) {
		nws.classList.add("splide","slider");
		nws.id = "splide-number-" + splidenumber;
	
		// track
		const eltrack = nws.querySelector(".wp-block-query");
		eltrack.classList.add("splide__track","alignfull");
	
		// list
		const ellist = nws.querySelector(".wp-block-post-template");
		ellist.classList.add("splide__list");

		slides = ellist.querySelectorAll("li");
		
		if( slides[0] ) {
			slides.forEach( function( sl ) {
				sl.classList.add("splide__slide");
			});
		}
		setupslider( nws, splidenumber);
...
function setupautosliding( el, splidenum ) {
	...
	elcontainer.className = el.className + " splide slider";
	elcontainer.id = "splide-number-" + splidenum;
	el.parentNode.insertBefore(elcontainer, el);

	// track
	const eltrack = document.createElement("div");
	eltrack.classList.add("splide__track");
	elcontainer.append( eltrack );
...
//	if( el.closest(".logos") ) {
		slides = el.querySelectorAll("figure");
//	}
	...
	if( slides[0] ) {
		slides.forEach( function( sl ) {
			...
			sli.classList.add("splide__slide");
			sli.append( sl );
//				sl.innerHTML = `<div class="wrapper"><p>${sl.innerHTML}</p></div>`;				
			ellist.append( sli );
		});
		sploptions = {
			type       : 'loop',
			...
			gap: 0,
			rewind: false,
			arrowPath: 'M18 6L16.57 7.393L24.15 15H4V17H24.15L16.57 24.573L18 26L28 16L18 6Z',
			easing: 'linear',
		...
		if( el.classList.contains("no-arrows") ) {
			sploptions.arrows = false;
			sploptions.gap = 20;
		}
		
		if( el.classList.contains("right") ) {
			sploptions.autoScroll.speed = -0.75;
		}

		sploptions.autoScroll.pauseOnHover = false;
		
		if( window.innerWidth < 1200 ) {
			sploptions.perPage = 1;
			sploptions.gap = 20;
		}
		el.remove();
		
		splide[splidenum] = new Splide( '#splide-number-' + splidenum, sploptions).mount( window.splide.Extensions );
	}
}
...
function getLastAllowedIndex(splideInstance) {
	// The total "content zone" width: from left content edge to right content edge
	const contentEl = document.querySelector('.is-layout-constrained');
	const contentRight = contentEl.getBoundingClientRect().right;
	const leftOffset = getContentLeftOffset();
	const availableWidth = contentRight - leftOffset;
	
	// Get actual slide width from the first slide element
	const firstSlide = splideInstance.Components.Elements.slides[0];
	const slideWidth = firstSlide.offsetWidth + 
		parseFloat(getComputedStyle(firstSlide).marginRight);
	
	// How many slides fit in the content zone?
	const visibleCount = Math.floor(availableWidth / slideWidth);
	
	// Last index where the first visible slide aligns left and last aligns right
	return splideInstance.length - visibleCount;
}
...
function setupslider(which, splid) {
	...
	if( window.innerWidth < 1024 ) {
		sploptions.perPage = 1;
		sploptions.perMove = 1;				
	}
	
	if( which.classList.contains("news-slider") ) {
		sploptions.arrows = false;
		sploptions.pagination = true;
		sploptions.perPage = 1;
		sploptions.interval = 8000;
	}
	
	if( which.classList.contains("left-offset") ) {
		const leftOffset = getContentLeftOffset();
		sploptions.trimSpace = false;
		sploptions.rewind = false;
		sploptions.padding = { left: leftOffset, right: 0 };
		sploptions.start = 0;
		sploptions.focus = 0;
	}
	
	which.id = "spl-" + splid;
	splide[splid] = new Splide( "#spl-" + splid, sploptions );
	
	if( which.classList.contains("left-offset") ) {
		const sp = splide[splid]; // shorthand to avoid repetition
	
		sp.on('move', function(newIndex, prevIndex, destIndex) {
			const lastAllowed = getLastAllowedIndex(sp);
			const nextBtn = document.querySelector('.splide__arrows .splide__arrow--next');
			if (newIndex >= lastAllowed) {
				setTimeout(() => nextBtn.disabled = true, 0);			
			}
			if (newIndex > lastAllowed) {
				// Override: snap back to last allowed
				setTimeout(() => sp.go(lastAllowed), 0);
				return;
			}
		});
	
		sp.on('moved', function(newIndex) {
			const lastAllowed = getLastAllowedIndex(sp);
			const prevBtn = sp.root.querySelector('.splide__arrows .splide__arrow--prev'); // sp.root
			const nextBtn = sp.root.querySelector('.splide__arrows .splide__arrow--next');
			if (newIndex >= lastAllowed) {
				// At the end: switch to right-aligned padding, disable next
//				sp.options = { padding: { left: 0, right: getContentLeftOffset() } };
				if (nextBtn) nextBtn.disabled = true;
				if (prevBtn) prevBtn.disabled = false;
				setTimeout(() => nextBtn.disabled = true, 0);

			} else if (newIndex === 0) {
				// At the start: disable prev
				sp.options = { padding: { left: getContentLeftOffset(), right: 0 } };
				if (prevBtn) prevBtn.disabled = true;
				if (nextBtn) nextBtn.disabled = false;
			} else {
				// Middle: both enabled, left-aligned
				sp.options = { padding: { left: getContentLeftOffset(), right: 0 } };
				if (prevBtn) prevBtn.disabled = false;
				if (nextBtn) nextBtn.disabled = false;
			}
//			sp.refresh();
		...
		sp.on('mounted', function() {
			...
			if (nextBtn) nextBtn.addEventListener('mousedown', function() {
				const lastAllowed = getLastAllowedIndex(sp);
				const currentIndex = sp.index;
				if (currentIndex + 1 >= lastAllowed) {
					setTimeout(() => nextBtn.disabled = true, 0);
				}
			});
		});
