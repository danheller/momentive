<?php
/**
 * patch-whitepaper-category-slugs.php
 *
 * Assigns correct categories to whitepaper posts whose legacy category slug
 * differs from the rebuilt site's slug. Posts with these legacy slugs were
 * skipped during migration (the category didn't exist under the old name):
 *
 *   event-technology  →  event-management   (14 whitepapers)
 *   career-services   →  career-centers     (5 whitepapers)
 *
 ...
 *
 *   wp eval-file migrations/patch-whitepaper-category-slugs.php --user=<admin>
 *     → dry run (default)
 *
 *   wp eval-file migrations/patch-whitepaper-category-slugs.php live --user=<admin>
 *     → applies changes
 *
 * Set MOMENTIVE_WW_LEGACY_WXR to override the default WXR path.
...
WP_CLI::log( '== patch-whitepaper-category-slugs ==' . ( $dry ? '  (DRY RUN)' : '  (LIVE)' ) );

$wxr = defined( 'MOMENTIVE_WW_LEGACY_WXR' )
	? MOMENTIVE_WW_LEGACY_WXR
	: __DIR__ . '/exports/momentivesoftware.whitepapers.current.2026-09-01.xml';

if ( ! file_exists( $wxr ) ) {
	WP_CLI::error( "Legacy WXR not found at {$wxr}. Set MOMENTIVE_WW_LEGACY_WXR." );
}
...
if ( preg_match_all( '#<item>(.*?)</item>#s', $xml, $items ) ) {
	foreach ( $items[1] as $item ) {
		if ( false === strpos( $item, '<wp:post_type><![CDATA[whitepaper]]>' ) ) {
			continue;
		}
...
WP_CLI::log( sprintf( 'Found %d whitepaper(s) in WXR with remappable categories.', count( $to_patch ) ) );
...
foreach ( $to_patch as $post_slug => $term_ids ) {
	$post = get_page_by_path( $post_slug, OBJECT, 'whitepaper' );
	if ( ! $post ) {
		WP_CLI::warning( "  Whitepaper not found in rebuilt site: '{$post_slug}' — skipped." );
		$missing++;
		continue;
	}
