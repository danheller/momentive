<?php
/**
 * migrate-infographics.php
 *
 * WP-CLI migration script: legacy `infographics` CPT → rebuilt `infographic` CPT.
 *
 * Reads from a single WXR export file (infographics posts + attachments).
 * Content lives entirely in the infographics export — no second assets file.
 *
 * Run (from the theme/migrations directory):
 *
 *   wp eval-file migrations/migrate-infographics.php --user=<admin>
 *     → dry run (default); shows what would be written
 *
 *   wp eval-file migrations/migrate-infographics.php live --user=<admin>
 *     → writes posts, sideloads media, sets ACF fields
 *
 *   wp eval-file migrations/migrate-infographics.php live limit=10 --user=<admin>
 *     → first 10 legacy infographics only
 *
 *   wp eval-file migrations/migrate-infographics.php live only=giving-tuesday-statistics --user=<admin>
 ...
 *   MOMENTIVE_INF_LEGACY_WXR   — path to legacy infographics export
 *   MOMENTIVE_INF_UPLOADS_BASE — base URL for resolving attachment IDs
 *
 ...
 *       → [GATED ONLY] checklist_title + checklist
 *       → resource_details_after_checklist (optional)
 *       → additional resource link button (optional)
 *
 *     Right column (.post-sidebar):
 *       [GATED]:
 *         post-featured-image → form heading paragraph
 *         (anchor="form" when additional_link is present) → acf/hubspot-form
 *
 *       [NOT GATED]:
 ...
 *
 *   After columns:
 *     Social-share (all posts).
 *
 * Hero image vs. featured image:
 *   _thumbnail_id         → featured image (archive card)
 *   resource_hero_image   → hero_image ACF field (singular-view hero override)
 *
 *   When _thumbnail_id and resource_hero_image are the same legacy attachment ID,
 *   the featured image covers both contexts and hero_image is left blank (no
 *   redundant sideload or field write). Posts #2 and #5 in the reference set
 *   exercise this path.
 */

if ( ! defined( 'WP_CLI' ) || ! WP_CLI ) {
	die( 'Run as: wp eval-file migrations/migrate-infographics.php [live] [limit=N] [only=slug] --user=<admin>' . PHP_EOL );
}
...
const MOMENTIVE_INF_CPT      = 'infographic';
const MOMENTIVE_INF_LEGACY   = 'infographics'; // post_type value in legacy WXR
const MOMENTIVE_INF_RUN_META = '_momentive_migration_run';

// ACF field key — hero_image (shared with Whitepaper Settings group_6a45de7a50be6;
// confirmed from the infographic rebuild WXR _hero_image meta on all 5 reference posts)
const FK_INF_HERO = 'field_6a45de7b50be7';

// ACF field keys — back-link block (group_6a44a4078d0f6)
const FK_INF_BL_LABEL = 'field_6a44a408f79e0';
const FK_INF_BL_URL   = 'field_6a44a420f79e1';

// ACF field keys — HubSpot form block
const FK_INF_HS_EMBED    = 'field_6a2873ba3bf87'; // hubspot_embed_code
const FK_INF_HS_TWO_STEP = 'field_6a35626f3a11b'; // two_step

/* -------------------------------------------------------------------------
 * Utilities
 * ---------------------------------------------------------------------- */

function momentive_inf_run_id(): string {
	static $id = '';
	if ( '' === $id ) {
 ...
 */
function momentive_inf_xml_tag( string $item, string $tag ): string {
	if ( preg_match(
 ...
 * <a> hyperlinks.
 */
function momentive_inf_strip_word( string $html ): string {
...
function momentive_inf_p_block( string $inner ): string {
	$inner = trim( $inner );
...
function momentive_inf_h_block( string $text, int $level = 2 ): string {
 ...
 */
function momentive_inf_html_to_blocks( string $html ): array {
	$html = trim( $html );
	if ( '' === $html ) {
		return [];
	}
	...
	if ( ! $root ) {
		return [ momentive_inf_p_block( wp_strip_all_tags( $html ) ) ];
	}
	...
	foreach ( $root->childNodes as $node ) {
		if ( $node->nodeType === XML_TEXT_NODE ) {
			...
			if ( '' !== $text ) {
				$blocks[] = momentive_inf_p_block( esc_html( $text ) );
			}
			continue;
		}
		...
		switch ( $tag ) {
			case 'p':
				...
				if ( '' !== $plain && '<br>' !== $text && '<br/>' !== $text ) {
					$blocks[] = momentive_inf_p_block( $text );
				}
				break;
			...
			case 'h6':
				...
				if ( '' !== $plain ) {
					$blocks[] = momentive_inf_h_block( $plain, $level );
				}
				break;
			...
			default:
				$plain = trim( wp_strip_all_tags( $inner_html ) );
				if ( '' !== $plain ) {
					$blocks[] = momentive_inf_p_block( esc_html( $plain ) );
				}
				break;
		}
	}
...
/** back-link block pointing to /infographics/ with full ACF data scaffold. */
function momentive_inf_back_link_block(): string {
	$attrs = wp_json_encode( [
		'name' => 'acf/back-link',
		'data' => [
			'label'  => 'All infographics',
			'_label' => FK_INF_BL_LABEL,
			'url'    => '/infographics/',
			'_url'   => FK_INF_BL_URL,
		],
		'mode' => 'preview',
 ...
 *
 * Infographics use a block-level embed (same pattern as whitepapers — no
 * post-level form fields). The full <script> embed code is written directly
 * into the block's data object so ACF can read it without a separate lookup.
 *
 * Encoding: JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP escapes <, >, ", &
 * as Unicode sequences, matching Gutenberg's JSON.stringify output and
 * preventing those characters from breaking the block comment's JSON.
 *
 * Slashing: post_content must be wrapped in wp_slash() before wp_update_post
 * — see the post content write below. wp_update_post calls wp_unslash()
 * internally; without wp_slash() first every backslash in the JSON is stripped.
 *
 ...
 */
function momentive_inf_hubspot_form_block( string $embed_code ): string {
	...
	$attrs = wp_json_encode( [
		...
		'data' => [
			FK_INF_HS_EMBED    => $embed_code,
			FK_INF_HS_TWO_STEP => '0',
		],
		'mode' => 'preview',
 ...
 */
function momentive_inf_button_block( string $url, string $text, bool $new_tab ): string {
	$url  = esc_url( $url );
	$text = esc_html( $text );
	if ( '' === $url || '' === $text ) {
		return '';
 ...
 *
 * Each row has a `description` key. Content is passed through wp_kses so that
 * items containing HTML anchor tags (e.g. the silent-auction-tips post, which
 * uses a resource-links checklist with full <a href target> markup) are
 * preserved as real links rather than escaped to plain text.
 *
 * Plain-text items are unaffected — wp_kses strips no meaningful content from
 * a string with no tags.
 */
function momentive_inf_checklist_block( array $items ): string {
	// Allow <a> with href, target, and rel; everything else is stripped safely.
	$allowed_tags = [
		'a' => [
			'href'   => [],
			'target' => [],
	...
	foreach ( $items as $row ) {
		...
		$safe        = wp_kses( $text, $allowed_tags );
...
/**
 * Assemble the full infographic page from left column, right column.
 * Social-share always appears after the columns (infographics have no
 * insights section — that field is a dead Elementor default on this CPT).
 */
function momentive_inf_page( string $content_col, string $sidebar_col ): string {
...
/**
 * Build legacy attachment-ID → fetchable-URL map from the WXR export.
 * Resolves legacy _thumbnail_id and resource_hero_image values to sideloadable URLs.
 */
function momentive_inf_build_attachment_map(): array {
	$path = defined( 'MOMENTIVE_INF_LEGACY_WXR' )
		? MOMENTIVE_INF_LEGACY_WXR
		: __DIR__ . '/momentivesoftware.infographics.current.2026-07-01.xml';
 ...
 */
function momentive_inf_sideload( string $url, int $post_id, bool $dry ): int {
	$url = trim( $url );
	if ( '' === $url ) {
		return 0;
	}
...
/**
 * Parse all legacy infographic post items from the WXR export.
 * Returns an array sorted by legacy post ID. Skips empty drafts.
 */
function momentive_inf_load_legacy_posts(): array {
	...
	$out = [];
	if ( ! file_exists( $path ) ) {
		WP_CLI::error( "Legacy WXR not found at {$path}. Place the export next to this script or set MOMENTIVE_INF_LEGACY_WXR." );
		return $out;
	}
	...
	foreach ( $all_items[1] as $item ) {
		if ( false === strpos( $item, 'post_type><![CDATA[infographics]]>' ) ) {
			continue;
		}

		$status = momentive_inf_xml_tag( $item, 'wp:status' ) ?: 'publish';

		// Skip the empty draft (1 post in the corpus has no content and is auto-draft).
		$title = momentive_inf_xml_tag( $item, 'title' );
		if ( 'auto-draft' === $status && '' === trim( $title ) ) {
			continue;
		}
		...
		$out[] = [
			'id'           => (int) momentive_inf_xml_tag( $item, 'wp:post_id' ),
			'title'        => $title,
			'slug'         => momentive_inf_xml_tag( $item, 'wp:post_name' ),
			'status'       => $status,
			'excerpt'      => $exc,
			'date'         => momentive_inf_xml_tag( $item, 'wp:post_date' ),
			'date_gmt'     => momentive_inf_xml_tag( $item, 'wp:post_date_gmt' ),
			'modified'     => momentive_inf_xml_tag( $item, 'wp:post_modified' ),
			'modified_gmt' => momentive_inf_xml_tag( $item, 'wp:post_modified_gmt' ),
			'meta'         => $meta,
			'cats'         => $cats,
		];
	}
 ...
 */
function momentive_inf_get_flags( array $argv = [] ): array {
	$flags = [
...
function momentive_inf_run( array $argv = [] ): void {
	$flags = momentive_inf_get_flags( $argv );
	$dry   = $flags['dry_run'];

	WP_CLI::log( '====================================================' );
	WP_CLI::log( '  Infographic migration' );
	...
	$attach_map = momentive_inf_build_attachment_map();
	$legacy_all = momentive_inf_load_legacy_posts();
	WP_CLI::log( sprintf( 'Legacy WXR: %d infographic items parsed.', count( $legacy_all ) ) );
	...
	if ( '' !== $flags['only'] ) {
		...
		if ( empty( $legacy_all ) ) {
			WP_CLI::error( "No legacy infographic found with slug \"{$slug_filter}\"." );
			return;
		}
	}
	if ( $flags['limit'] > 0 ) {
	...
	$summary = [
		...
		'hero_imported'       => 0,
		'hero_skipped_same'   => 0, // thumbnail and hero were the same legacy ID
		'gated'               => 0,
		'not_gated'           => 0,
		'additional_links'    => 0,
		'cats_linked'         => 0,
	...
	foreach ( $legacy_all as $legacy ) {
		...
		// Note: infographics default to NOT gated (8/14 are ungated).
		$enable_gated = filter_var(
			$m['enable_gated_content'] ?? 'false',
		...
		// ---- HubSpot embed code (gated posts only) --------------------------
		...
		// ---- Content fields -------------------------------------------------
		$resource_details  = momentive_inf_strip_word( (string) ( $m['resource_details']  ?? '' ) );
		...
		$after_checklist   = momentive_inf_strip_word( (string) ( $m['resource_details_after_checklist'] ?? '' ) );
		...
		$form_heading      = trim( (string) ( $m['form_heading'] ?? 'Download the infographic' ) );

		// ---- Legacy attachment IDs ------------------------------------------
		$thumb_legacy_id = (int) ( $m['_thumbnail_id']        ?? 0 );
		$hero_legacy_id  = (int) ( $m['resource_hero_image']  ?? 0 );
		$same_image      = ( $hero_legacy_id > 0 && $hero_legacy_id === $thumb_legacy_id );

		// ---- Dry-run summary ------------------------------------------------
		if ( $dry ) {
			$hero_url  = ( $hero_legacy_id && isset( $attach_map[ $hero_legacy_id ] ) ) ? $attach_map[ $hero_legacy_id ] : '';
			$thumb_url = ( $thumb_legacy_id && isset( $attach_map[ $thumb_legacy_id ] ) ) ? $attach_map[ $thumb_legacy_id ] : '';
			if ( $hero_legacy_id && '' === $hero_url && ! $same_image ) {
				$media_unresolved[] = "{$title}: hero ID {$hero_legacy_id} not in attachment map";
			}
			if ( $thumb_legacy_id && '' === $thumb_url ) {
				$media_unresolved[] = "{$title}: thumbnail ID {$thumb_legacy_id} not in attachment map";
			}
			WP_CLI::log( sprintf(
				'    [dry-run] gated=%-3s add_link=%-3s checklist_items=%d same_image=%-3s hero=%s',
				$enable_gated           ? 'yes' : 'no',
				$enable_additional_link ? 'yes' : 'no',
				count( $checklist_items ),
				$same_image             ? 'yes' : 'no',
				( '' !== $hero_url && ! $same_image ) ? 'yes' : ( $same_image ? 'skip(same)' : 'no' )
			) );
			if ( $enable_additional_link ) { $summary['additional_links']++; }
			if ( $enable_gated ) { $summary['gated']++; } else { $summary['not_gated']++; }
			if ( $same_image )   { $summary['hero_skipped_same']++; }
			continue;
		}
		...
		if ( empty( $existing ) ) {
			update_post_meta( $new_id, MOMENTIVE_INF_RUN_META, momentive_inf_run_id() );
		}
		...
		if ( $thumb_legacy_id ) {
			if ( isset( $attach_map[ $thumb_legacy_id ] ) ) {
				$thumb_att = momentive_inf_sideload( $attach_map[ $thumb_legacy_id ], $new_id, false );
				if ( $thumb_att > 0 ) {
		...
		// Only set when different from the featured image. When _thumbnail_id and
		// resource_hero_image are the same legacy attachment ID, the featured image
		// already handles both the archive card and the singular hero, so
		// hero_image is correctly left blank. Posts #2 and #5 in the 5-post
		// reference set exercise this path (same-ID case confirmed in rebuild WXR).
		if ( $hero_legacy_id && ! $same_image ) {
			if ( isset( $attach_map[ $hero_legacy_id ] ) ) {
				$hero_att = momentive_inf_sideload( $attach_map[ $hero_legacy_id ], $new_id, false );
				if ( $hero_att > 0 ) {
			...
			WP_CLI::log( "    hero_image: skipped (same attachment as thumbnail — #{$thumb_legacy_id})" );
			$summary['hero_skipped_same']++;
		}
		...
		// Posts with no category (e.g. giving-tuesday-statistics) are valid —
		// leave the category panel empty, do not assign a default.

		// ---- Build post content ---------------------------------------------

		// The anchor="form" attribute is added to the right-column heading/paragraph
		// whenever a left-column button needs to link to #form as a scroll target.
		// For gated posts with an additional link, the form heading gets the anchor.
		// For all not-gated posts, the checklist_title paragraph gets the anchor.
		$needs_form_anchor = $enable_additional_link || ! $enable_gated;

		// Left column.
		$content_parts   = [];
		$content_parts[] = momentive_inf_back_link_block();
		$content_parts[] = '<!-- wp:query-title {"type":"post-type","showPrefix":false,"className":"top-label"} /-->';
		$content_parts[] = '<!-- wp:post-title {"level":1} /-->';

		foreach ( momentive_inf_html_to_blocks( $resource_details ) as $b ) {
			$content_parts[] = $b;
		}
		...
		if ( '' !== $details_cta ) {
			...
			$content_parts[] = momentive_inf_p_block( '<strong>' . esc_html( $details_cta ) . '</strong>' );
		}

		if ( $enable_gated ) {
			// Gated path: checklist goes in the left column.
			if ( '' !== $checklist_title ) {
				$content_parts[] = momentive_inf_p_block( '<strong>' . esc_html( $checklist_title ) . '</strong>' );
			}
			$clist = momentive_inf_checklist_block( $checklist_items );
			if ( '' !== $clist ) {
				$content_parts[] = $clist;
			}
		}
		// Not-gated path: checklist goes in the right column (see below).

		// resource_details_after_checklist always goes in the left column,
		// after the (optional) left-column checklist and before the extra button.
		foreach ( momentive_inf_html_to_blocks( $after_checklist ) as $b ) {
			$content_parts[] = $b;
		}

		if ( $enable_additional_link && '' !== $resource_link && '' !== $resource_link_txt ) {
			$content_parts[] = momentive_inf_button_block( $resource_link, $resource_link_txt, $resource_new_tab );
		...
		if ( $enable_gated ) {
			...
			if ( '' !== $form_heading ) {
				if ( $needs_form_anchor ) {
					...
					$sidebar_parts[] = momentive_inf_p_block( '<strong>' . esc_html( $form_heading ) . '</strong>' );
				}
			}
			$sidebar_parts[] = momentive_inf_hubspot_form_block( $raw_form );
			$summary['gated']++;
		} else {
			...
			if ( '' !== $resource_link && '' !== $resource_link_txt ) {
				$sidebar_parts[] = momentive_inf_button_block( $resource_link, $resource_link_txt, $resource_new_tab );
			}
			$summary['not_gated']++;
		}
		...
		$post_content = momentive_inf_page( $left_col, $right_col );
		...
		WP_CLI::log( "    wrote infographic #{$new_id}" );
	}
...
momentive_inf_run( isset( $args ) && is_array( $args ) ? $args : [] );
