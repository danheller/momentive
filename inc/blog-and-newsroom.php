...
add_filter( 'render_block', function( $block_content, $block ) {
	...
	$block_content = preg_replace( '/<img[^>]+>/', $replacement, $block_content, 1 );

	// Mark the wrapper so CSS can distinguish hero_image from the post thumbnail.
	$block_content = preg_replace( '/class="([^"]*wp-block-post-featured-image[^"]*)"/', 'class="$1 has-hero-image"', $block_content, 1 );
...
/**
 * Increase the default excerpt length.
 */

function momentive_excerpt_length( $length ) {
	return 256;
}

add_filter( 'excerpt_length', 'momentive_excerpt_length', 999 );