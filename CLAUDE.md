...
- Child solutions inherit `accent_color` from parent for front-end rendering (`wp_head`, `inc/solutions.php`); the field is hidden on child posts in the single-post editor. The Solutions list table's "Accent Color" column, however, deliberately does **not** walk up to the parent — it shows a value only when the field is actually set on that exact row, so the admin list reflects real per-post data rather than the resolved/inherited display color
